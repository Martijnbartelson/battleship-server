...
const checkCoordinates = (coordinate,length,direction,symbol,grid,boolean=true) => { 
    if(length===0 || boolean === false){ return boolean }
...
const placeShipOnGrid = (grid,ship,shipIsPlaced=false) => {
    if(shipIsPlaced){ return grid }

    let startCoordinate = randomCoordinate(ship) 
    let direction = randomDirection()
    if(startCoordinate[0] > (12 - ship.length) ) { direction = 'right'}
    if(startCoordinate[1] > (12 - ship.length) ) { direction = 'down'}
...
export const makeUserMove = (game, coordinate) => {
...
export const checkFinished = (game) => {
